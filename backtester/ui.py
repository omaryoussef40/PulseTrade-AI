...
def render_strategy_lab_tab(config: dict, default_symbols: list[str]):
    ...
    st.caption("v0.9.5 conservative Yahoo pricing: equity compounding stays on, but option exits are capped by a more realistic pricing model.")
    ...
    st.markdown("### 1. Backtest Setup")
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        period = st.selectbox("Historical period", ["5d", "10d", "30d", "60d"], index=2, key="sl_period")
    with c2:
        interval = st.selectbox("Candle interval", ["5m", "15m", "30m", "60m"], index=0, key="sl_interval")
    with c3:
        max_symbols = st.number_input("Max symbols", min_value=1, max_value=20, value=min(5, max(1, len(default_symbols))), step=1, key="sl_max_symbols")
    with c4:
        force_refresh = st.checkbox("Force Yahoo refresh", value=False, key="sl_force_refresh")

    default_text = ", ".join(default_symbols[: int(max_symbols)]) if default_symbols else "SPY, QQQ, NVDA, TSLA, AMD"
    symbols_text = st.text_area("Symbols", value=default_text, height=80, key="sl_symbols")
    symbols = _parse_symbols(symbols_text, int(max_symbols))

    with st.expander("Replay and scanner rules", expanded=False):
        r1, r2, r3, r4 = st.columns(4)
        with r1:
            orb_minutes = st.number_input("ORB window minutes", min_value=5, max_value=90, value=30, step=5, key="sl_orb")
        with r2:
            first_signal_minutes = st.number_input("First scanner minute", min_value=5, max_value=120, value=35, step=5, key="sl_first_signal")
        with r3:
    ...
    st.markdown("### 2. Run Strategy Lab")
    a, b, c = st.columns([2, 1, 1])
    run_clicked = a.button("▶ Run Full Backtest", use_container_width=True, key="sl_run")
    ...
    if show_cache:
        info = YahooDataClient().cache_info()
        st.dataframe(info, use_container_width=True, hide_index=True) if not info.empty else st.info("No Yahoo cache files yet.")
    ...
    if run_clicked:
        ...
        try:
            ...
            status.success("Strategy Lab backtest complete. Results saved to backtester/exports.")
    ...
    if isinstance(replay, pd.DataFrame) and not replay.empty:
        ...
        st.caption(f"Completed: {meta.get('completed_at', 'N/A')} | Symbols: {', '.join(meta.get('symbols', []))} | Period: {meta.get('period', 'N/A')} | Interval: {meta.get('interval', 'N/A')}")
